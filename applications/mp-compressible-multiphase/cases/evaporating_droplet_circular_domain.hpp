...
namespace MeltPoolDG::Simulation::CompressibleMultiphase
{
   ...
   */
  template <int dim, typename number>
  class SimulationEvaporatingDropletCircularDomain final : public Multiphase::CompressibleMultiphaseCase<dim, number>
  {
  public:
    /**
     * @brief Constructor.
     ...
     */
    explicit SimulationEvaporatingDropletCircularDomain(std::string parameter_file, const MPI_Comm mpi_communicator)
    ...
    create_spatial_discretization() override
    {
      ...
      constexpr number radius = 500.e-6;

      dealii::GridGenerator::hyper_ball_balanced(
        *this->triangulation, center, radius);
    ...
    set_boundary_conditions() override
    {
      ...
      const auto [bc1, bc2, bc3, bc4, bc5, bc6] =
        get_colorized_rectangle_boundary_ids<dim>();

      attach_boundary(0, boundary_condition);
    ...
    add_simulation_specific_parameters(dealii::ParameterHandler &prm) override
    {
      ...
      {
        prm.add_parameter("boundary condition",
                          boundary_condition,
  ...
  private:
    ...
    std::string boundary_condition  = "no_slip_wall";
