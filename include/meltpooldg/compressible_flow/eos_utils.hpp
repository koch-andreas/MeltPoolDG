...
namespace MeltPoolDG::CompressibleFlow::EOS
{
   ...
   */
  template <int dim, typename number>
  class WeaklyCompressible : public EquationOfStateUtils<dim, number>
  {
  public:
    /**
     * @brief Constructor.
     ...
     * parameters.
     */
    explicit WeaklyCompressible(const MaterialPhaseData<number> &material_data_in)
      : material_data(material_data_in)
    {}
    ...
    inline DEAL_II_ALWAYS_INLINE //
      ...
      calculate_thermodynamic_pressure(
        const ConservedVariablesType<dim, number> &conserved_variables) const override
    {
      return material_data.eos_data.artificial_sound_speed * material_data.eos_data.artificial_sound_speed
         * (conserved_variables[0] - material_data.eos_data.linearization_point_density) + material_data.eos_data.linearization_point_pressure;
    }
        ...
        const ConservedVariablesGradientType<dim, number> &grad_conserved_variables) const override
    {
      ...
      return grad_E / material_data.specific_isobaric_heat;
    }
    ...
    inline DEAL_II_ALWAYS_INLINE //
      ...
      calculate_speed_of_sound(
        const ConservedVariablesType<dim, number> &conserved_variables) const override
    {
      return material_data.eos_data.artificial_sound_speed;
    }
        ...
        const ConservedVariablesType<dim, number> &u_prim) const override
    {
      ...
      u_cons[0] = u_prim[0] / (material_data.eos_data.artificial_sound_speed * material_data.eos_data.artificial_sound_speed)
        + material_data.eos_data.linearization_point_density;
      ...
      const dealii::Tensor<1, dim, dealii::VectorizedArray<number>> velocity =
        calculate_velocity<dim, number>(u_cons);
      u_cons[dim + 1] =
        u_cons[0] * (material_data.specific_isobaric_heat * u_prim[dim + 1] + 0.5 * scalar_product(velocity, velocity));
