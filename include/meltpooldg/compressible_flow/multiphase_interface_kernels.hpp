...
namespace MeltPoolDG::Multiphase
{
      ...
      const number                                                        &laser_heat_source)
  {
    ...
    const dealii::VectorizedArray<number> stress_tensor_liquid =
      CompressibleFlow::calculate_stress_tensor(liquid_state.pressure(),
                                                viscous_stress_tensor_liquid)[0][0];
    const dealii::VectorizedArray<number> stress_tensor_gas =
      CompressibleFlow::calculate_stress_tensor(gas_state.pressure(),
      ...
      const number                                                        &delta_T)
  {
    ConservedVariablesType liquid_prim_data =
      CompressibleFlow::conservative_to_primitive<dim, ConservedVariablesType>(liquid_state);
    ...
    J_Dir[Idx::pressure] = delta_p - m_dot_evap * (liquid_prim.velocity(0) - gas_prim.velocity(0));
    J_Dir[Idx::velocity] = m_dot_evap * (1. / liquid_state.density() - 1. / gas_state.density());
    J_Dir[Idx::temperature] = delta_T;

    const auto liquid_prim_tmp = liquid_prim_data;

    liquid_prim_data = gas_prim_data + J_Dir;
    ...
    const auto u_gas_cons_star =
      CompressibleFlow::primitive_to_conservative<dim, ConservedVariablesType>(gas_prim_mod);
